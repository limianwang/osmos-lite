...
Document.prototype.get = function getValueFromDocument(target, name) {
  if (name in this) {
    ...
    if (typeof result == 'function' && !fieldsToIgnore[name]) result = result.bind(Proxy(this.data, this));

    return result;
  }
  
  if (name in this.model.instanceMethods) return this.model.instanceMethods[name].bind(Proxy(this.data, this));
  
  if (name in this.model.instanceProperties) return this.instanceProperties[name].bind(Proxy(this.data, this));
...
Document.prototype.save = function(cb) {
  ...
  this.model._save(Proxy(this.data, this), function(err) {
...
Document.prototype.del = function(cb) {
  this.model._delete(Proxy(this.data, this), cb);
};
