...
var Model = function Model(name, schema, bucket, db, documentClass) {
  ...
  this.generateDocumentClass(documentClass || Document);
...
Model.prototype.generateDocumentClass = function(documentClass) {
  var docClass = function OsmosDataStoreDocument() {
    documentClass.apply(this, arguments);
  };

  util.inherits(docClass, documentClass);

  this.documentClass = docClass;
};

Model.prototype.initializeDocumentClass = function() {
  var key, source;
  var docClass = this.documentClass;
  var prop;

  docClass.virtualPropertyValues = {};

  source = this.instanceProperties
  for (key in source) {
    prop = source[key];

    if (prop.set || prop.get) Object.defineProperty(docClass.prototype, key, source[key]); else docClass.virtualPropertyValues[key] = prop;
  }

  source = this.instanceMethods;
  for (key in source) {
    docClass.prototype[key] = source[key];
  }

  function loadVirtualProperties(source) {
    for (key in source) {
      docClass.virtualProperties[key] = '_' + key;
  ...
  docClass.virtualProperties = {};
  loadVirtualProperties(this.transformers);
  loadVirtualProperties(this.schema.transformers);

  Object.seal(this.instanceMethods);
  Object.seal(this.instanceProperties);
  Object.seal(this.transformers);

  this.hasInitializedDocumentClass = true;
}

Model.prototype._initialize = function(data, cb) {
  if (!this.hasInitializedDocumentClass) this.initializeDocumentClass();
