  ...
  async.waterfall([
    ...
    function(data, cb) {
      self._initialize(data, function(err, doc) {
        ...
        self.callHook('didCreate', doc).then(function() {
          cb(null, doc);
        }).catch(function(err) {
          cb(err, null);
        });
...
Model.prototype._delete = function(doc, cb) {
  if (!doc.primaryKey) {
    return Promise.reject(new OsmosError('This document does not have a primary key')).nodeify(cb);
  }
