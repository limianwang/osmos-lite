...
Schema.validateSchema = function validateSchema(schema) {
  if (!schema) throw new Error('Empty or non-existent schema passed to Osmos.Schema.validateSchema()');
  ...
  if (result.missing.length) throw new Error('Invalid or unknown schema', result.missing);
  if (result.errors.length) throw new Error('Failed to validate schema.', result.errors);
}
...
Schema.prototype.validateDocument = function(doc, cb) {
  ...
  this.performHookCycle(
    ...
    function validateSchema(cb) {
      var payload = doc.toJSON ? doc.toJSON() : doc;
