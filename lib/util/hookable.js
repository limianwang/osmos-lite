...
var Hookable = function Hookable() {
  ...
  if (!this.hooks) throw new Error('No hooks are defined in this object.');
  
  this.hookCallbacks = {};
  this.hookMap = this.hooks.reduce(
    function(memo, key) { memo[key] = 1; return memo; },
    {}
  );
};
...
Hookable.prototype.callHook = function callHook(hook, args, cb) {
  if (!this.hookMap[hook]) throw new Error('The hook `' + hook + '` is not defined.');
  
  if (!this.hookCallbacks[hook] || !this.hookCallbacks[hook].length) return cb(null);
...
Hookable.prototype.hook = function hookF(hook, callback) {
  ...
  if (!this.hookCallbacks[hook]) this.hookCallbacks[hook] = [];
...
Hookable.prototype.unhook = function unhook(hook, callback) {
  ...
  if (index != -1) throw new Error('The callback `' + callback + '` is not registered.');
