...
Driver.prototype = {
    ...
    post : function post(bucket, document, data, callback) {
        r
            ...
            .run(this.connection, function(err, result) {
                ...
                if (result.errors.length) return callback(new Error('Database insertion error: ' + JSON.stringify(result.errors)));
    ...
    put : function put(bucket, document, data, callback) {
        ...
        if (document.originalData[document.schema.primaryKey] == undefined) {
            return this.post(bucket, document, data, callback);
        }

        var changes = objectDiff.diff(document.originalData, data);
        ...
        if (diff[document.schema.primaryKey]) {
            primaryKey = document.originalData[document.schema.primaryKey];
        } else {
