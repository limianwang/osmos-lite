...
var documentProxy = {
    ...
    get : function osmosProxyGet(target, name, receiver) {
        switch(name) {
            ...
            default:
                if (name in target.model.schema.fields) {
                    return target.methods.getFieldValue.call(target, name);
                }
                ...
                if (name in target.properties) {
                    return target.properties[name].get.call(target);
                }
                
                if (name in allowableProperties) {
                    return target[name];
                }
        }
    ...
    set : function osmosProxySet(target, name, value, receiver) {
        if (name in target.model.schema.fields) {
            return target.methods.setFieldValue.call(target, name, value);
        }
        
        if (name in target.properties) {
            target.properties[name].set.call(target, value);
        }
...
Document.prototype.methods = {
    ...
    setFieldValue: function osmosDocumentSetFieldValue(fieldName, value) {
    ...
    getFieldValue: function osmosDocumentGetFieldValue(fieldName) {
